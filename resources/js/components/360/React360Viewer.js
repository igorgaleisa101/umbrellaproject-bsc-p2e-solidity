...
import Button from './React360Button'
import './React360ViewerStyle.css'
...
class React360Viewer extends Component {
    ...
    fetchData(){
        for(let i=1; i <= this.props.amount; i++){
            ...
            const fileName = this.props.fileName.replace('{index}', this.convertFixedString(imageIndex));
            // const filePath = `${this.props.imagePath}/${fileName}`
            const filePath = this.props.imagePath.replace('xxxx', fileName);
